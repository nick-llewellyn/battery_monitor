# Copilot instructions for `battery_status`

`battery_status` is a Flutter plugin that exposes battery level,
...
- Native: `android/src/.../BatteryStatusPlugin.kt` and
  `ios/battery_status/Sources/battery_status/BatteryStatusPlugin.swift`
  register the three EventChannels and back them with `BroadcastReceiver`
  / `NotificationCenter` observers. The iOS sources are shared by the
  SPM `Package.swift` and the CocoaPods `ios/battery_status.podspec`.

Channel names are namespaced under
`com.nllewellyn.battery_status/`. **The native registration and the
