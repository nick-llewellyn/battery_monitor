...
// in, which is set to 5.0 in `ios/battery_status.podspec`
...
// Xcode 15+.
//
// SPM manifest for the iOS plugin. Mirrors `ios/battery_status.podspec`
// (iOS 13.0); both build paths consume the same sources under
// `Sources/battery_status/`. Flutter's SPM integration auto-links the
...
let package = Package(
    ...
    products: [
        ...
        .library(name: "battery-status", targets: ["battery_status"])
    ],
    dependencies: [],
    targets: [
        .target(
